    ...
    <div className="flex flex-col gap-4 pt-safe pb-[calc(env(safe-area-inset-bottom,0px)+2rem)]">
      ...
      <section className="rounded-3xl border-2 border-on-surface/10 bg-surface-container-lowest p-space-md shadow-soft">
        ...
        <button
          ...
          className="mt-2 flex h-11 w-full items-center justify-center gap-2 rounded-full bg-secondary text-label-md text-white transition-all active:scale-[0.98] disabled:opacity-50"
        >
            ...
            {CATEGORIES.map((c) => (
              <button
                ...
                className={`h-9 shrink-0 rounded-full px-3.5 text-label-sm capitalize transition-all ${
                  category === c ? "bg-secondary text-white" : "bg-surface-container text-on-surface-variant"
                }`}
              >
                {c}
              </button>
            ...
            {DURATIONS.map((d) => (
              <button
                ...
                className={`h-11 rounded-full text-label-md transition-all ${
                  durationDays === d ? "bg-secondary text-white shadow-sm" : "bg-surface-container text-on-surface-variant"
                }`}
              >
                {d}d
              </button>
          ...
          <div className="grid grid-cols-4 gap-space-xs">
            {[1, 2, 3, 5].map((n) => (
              <button
                ...
                className={`h-11 rounded-full text-label-md transition-all ${
                  maxPosts === n ? "bg-secondary text-white shadow-sm" : "bg-surface-container text-on-surface-variant"
                }`}
              >
                {n}
              </button>
