          ...
          {[right, left].map((name) =>
            Array.from({ length: cards }, (_, i) => {
              ...
              return (
                ...
                >
                  {img ? (
                    // Eager, not lazy: the cards are heavily 3D-transformed, so
                    // the browser's lazy-load heuristic treats them as
                    // off-viewport and never fetches them — leaving the
                    // corridor empty. The image set is the whole point here.
                    <img
                      src={img.src}
                      alt={img.alt ?? ""}
                      loading="eager"
