...
import { Camera, ChevronRight, Clock, Flag, Hourglass, Loader2, Trophy, Users, Vote } from "lucide-react";
...
export function CampaignDetail({ id }: { id: string }) {
  ...
  const canPost = live && remaining !== 0;
  ...
  const stats = [
    ...
    { icon: Vote, value: compactNumber(campaign.stats.votes), label: "Votes" },
  ];
  ...
  return (
    <>
      ...
      <main className="flex flex-1 flex-col gap-4 pb-[calc(env(safe-area-inset-bottom,0px)+7rem)]">
        ...
        <div className="flex flex-col gap-4 px-4">
          <div className="flex items-center justify-between gap-2">
            {campaign.prizePool > 0 ? (
              <span className="flex items-center gap-1.5 rounded-full bg-primary-container px-3 py-1.5 text-label-md font-bold text-on-primary-container">
            ...
            <ol className="flex flex-col gap-2">
              {podium.map((e) => (
                <li key={e.post.id}>
                  ...
                  >
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary-container text-label-sm font-extrabold text-on-primary-container">
                      {e.rank}
                    </span>
        ...
        {isHost && live && (
          <section className="rounded-3xl border-2 border-error/20 bg-surface-container-lowest p-space-md shadow-soft">
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-0 z-40 bg-gradient-to-t from-surface via-surface/90 to-transparent px-4 pt-6 pb-[calc(env(safe-area-inset-bottom,0px)+0.75rem)]">
        <div className="app-shell pointer-events-auto">
          {canPost ? (
            ...
            >
              <Camera size={24} />
              {remaining !== null && remaining < campaign.maxPostsPerUser ? `Post another snap (${remaining} left)` : "Join & Snap"}
            </Link>
          ) : (
            <Link
              href={`/campaigns/${id}/leaderboard`}
              className="flex h-14 w-full items-center justify-center gap-space-sm rounded-full bg-primary-container text-headline-sm text-on-primary-container shadow-lg transition-all active:scale-[0.98]"
            >
              {live ? <Hourglass size={22} /> : <Trophy size={22} />}
              {live ? "Snap limit reached · View rankings" : "See winners"}
            </Link>
          )}
        </div>
      </div>
